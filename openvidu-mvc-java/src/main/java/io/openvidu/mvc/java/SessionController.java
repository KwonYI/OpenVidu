 ...
 *     3) session : 방 세션 - io.openvidu.java.client.Session@~~~~~ 이런식으로 생겼다
 ...
 *         - 세션아이디                          sessionId=ses_~~~~~&   -> 방이 같으면 동일해
 *           참가자 토큰                         token=tok_~~~~~~~~~&   -> 참가자 토큰이기때문에 다 달라
 ...
 *      -> 연결된 세션 아이디( 방 아이디 ) + 토큰( 연결에 대한 고유값 ) + 연결 역할(퍼블리셔, 모더레이터 )
 *   4. 토큰 정보와 방 이름( 이 예제에선 이름이 다르면 다른 방이기떄문에 방이름을 넣어준다) model에 넣어줘
	...
	public String joinSession(@RequestParam(name = "data") String clientData,
		...
		if (this.mapSessions.get(sessionName) != null) {
			try {
				 ...
				 * 입장할 사람의 토큰
				 * wss://localhost:4443?
				 * sessionId=ses_~~~~~ 방의 토큰값                 -> 이미 있는 세션의 경우 동일
				 * &token=tok_~~~~~~~~ 입장한 사람의 토큰값      -> 입장한 사람마다 달라
				...
				// 같은 방에 접속된 사람들 정보에 넣어준다 mapSessionNamesTokens - 방 세션 : {입장한 사람들의 토큰 : 역할}
				this.mapSessionNamesTokens.get(sessionName).put(token, role);

				// model에는 방이름과 방에 들어간 사람의 토큰값, 그 사람의 원래 아이디와 입장 시 작성한 아이디가 들어가
